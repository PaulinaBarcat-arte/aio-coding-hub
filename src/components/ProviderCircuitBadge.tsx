...
export function ProviderCircuitBadge({
  ...
  return (
    <Popover
      ...
      contentClassName="w-[480px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card"
    >
      <div className="border-b border-slate-200 px-4 py-3">
        <span className="text-sm font-semibold text-slate-900">熔断列表 ({count})</span>
      </div>
          ...
          .map((cliKey) => (
            <div key={cliKey} className="mb-3 last:mb-0">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="text-xs text-slate-500">{groupedByCli[cliKey].length} 个熔断</span>
              </div>
                ...
                {groupedByCli[cliKey].map((row) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50/50 px-3 py-2 transition-colors hover:bg-slate-100"
                    >
                      ...
                      <div className="shrink-0 font-mono text-xs text-slate-500">{remaining}</div>
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={(e) => {
