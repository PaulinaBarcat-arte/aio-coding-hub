...
mod imp {
    ...
    impl Notification {
        ...
        pub fn show(self) -> crate::Result<()> {
            ...
            #[cfg(target_os = "macos")]
            {
                // Always use the app identifier so the notification icon matches the app (including dev/debug).
                // Note: This intentionally differs from upstream behavior, which defaults to Terminal in dev mode.
