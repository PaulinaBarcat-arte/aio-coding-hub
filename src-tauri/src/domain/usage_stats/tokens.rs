...
pub(super) fn sql_effective_input_tokens_expr_with_alias(alias: &str) -> String {
    format!(
        "CASE WHEN {alias}.cli_key IN ('codex','gemini') THEN MAX(COALESCE({alias}.input_tokens, 0) - COALESCE({alias}.cache_read_input_tokens, 0), 0) ELSE COALESCE({alias}.input_tokens, 0) END"
    )
}
